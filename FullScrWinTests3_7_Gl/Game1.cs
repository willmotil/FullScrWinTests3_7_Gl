...
{
    // this is actually test version 11
    public class Game1 : Game
    {
        ...
        // best match display mode.
        string bestMode = "";

        // help
        string helpMsg = (nline + "Help options list: H or O" + nline + nline);
        ...
        protected override void LoadContent()
        {
            ...
            DisplayOptions();
            string temp = displayedMsg + optionsMsg;
            displayedMsg =
                "This is a testing demo for MonoGames Fullscreen and Window capabilitys \n" +
                "This app may also be helpful in debuging \n" +
                "To see more detailed information \n" +
                "Turn on the projects console window in  \n" +
                "project > properties > application > type output >  \n"
                + nline + nline + temp;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                displayedMsg = ndnline + helpMsg + "INPUT  F1   Setting gdm preferred backbuffer" + preset + nline;
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                SetPreferredBackBufferThruGdm(preset.X, preset.Y);
                ApplyTheChanges();
            }
            if ((kbs.IsKeyDown(Keys.F2) || auto_script_cmd_value == 2) && Paused == false)
            {
                displayedMsg = ndnline + helpMsg + "INPUT  F2   Setting gdm preferred backbuffer " + preset2 + nline;
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
            ...
            {
                displayedMsg = ndnline + helpMsg + "INPUT  F3   set backbuffer thru gd presentation parameters  " + preset2 + " (to a non standard resolution)" + nline;
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
            ...
            {
                displayedMsg = ndnline + helpMsg + "INPUT  F4   List supported display modes" + nline;
                displayedMsg += nline + "     ___________________________" + nline;
                displayedMsg += nline + "     Current display mode index  " + FindCurrentDisplayModeInList().ToString() + nline;
                ListSupportedDisplayModes();
                FindBestDisplayModes();
                displayedMsg += supportedModes + ndnline;
                displayedMsg += "BestMatchingMode: \n"+ bestMode;
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
            }
            if ((kbs.IsKeyDown(Keys.F5) || auto_script_cmd_value == 6) && Paused == false)
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  F5  set the backbuffer to the current DisplayModeAdapterWidthHeight " + nline);
                SetPreferredBackBufferThruGdm(GraphicsDevice.DisplayMode.Width, GraphicsDevice.DisplayMode.Height);
                ApplyTheChanges();
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
            }
            ...
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  F6   Go into fullscreen -- Setting gdm preferred backbuffer" + started + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                SetFullScreen(true);
            ...
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  F7   Backout of fullscreen -- Setting gdm preferred backbuffer" + started + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                SetFullScreen(false);
            ...
            if ((kbs.IsKeyDown(Keys.F8) || auto_script_cmd_value == 8) && Paused == false)
            {
                ...
                ApplyTheChanges();
                displayedMsg = (ndnline + helpMsg + "INPUT  F8    HardwareModeSwitch: " + gdm.HardwareModeSwitch + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += RecordDiff.DisplayCurrentStatus(gdm, gd, Window);
                displayedMsg += ndnline + optionsMsg;
            }
            ...
            if ((kbs.IsKeyDown(Keys.F9) || auto_script_cmd_value == 9) && Paused == false)
            {
                ...
                ListSupportedDisplayModes();
                FindBestDisplayModes();
                int currentdisplayindex = FindCurrentDisplayModeInList();
                displayedMsg = 
                    (
                    ndnline + helpMsg + "INPUT  F9   Change displaymode. \n" +
                    "Index: " + currentdisplayindex.ToString() + 
                    "\n" +"Hardwaremode "+ gdm.HardwareModeSwitch + nline
                    );
                Console.WriteLine(displayedMsg);
                displayedMsg += supportedModes;
                displayedMsg += RecordDiff.DisplayCurrentStatus(gdm, gd, Window);
                displayedMsg += ndnline + optionsMsg;
            }
            ...
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  F10   set to fullscreen via gdm applychanges" + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                SetFullScreen(true);
                ApplyTheChanges();
            ...
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  F11   set to windowed   via gdm applychanges" + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                SetFullScreen(false);
                ApplyTheChanges();
            ...
            {
                displayedMsg = ndnline + helpMsg + "INPUT  F12   toggle fullscreen (their is no call to apply changes like this)" + nline;
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                TheFullScreenToggle();
            ...
            {
                displayedMsg = (ndnline + helpMsg + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
                DisplayOptions();
            }
            ...
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  I   current display Information" + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg = RecordDiff.DisplayCurrentStatus(gdm, gd, Window);
                displayedMsg += ndnline + optionsMsg;
            }
            ...
            {
                displayedMsg = (ndnline + helpMsg + "INPUT  T   run auto Test Script" + nline);
                Console.WriteLine(displayedMsg);
                displayedMsg += ndnline + optionsMsg;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.DrawString(mgFont, displayedMsg , new Vector2(10, 50), Color.White);
            spriteBatch.End();
            base.Draw(gameTime);
        }
        ...
        public void DisplayOptions()
        {
            ...
            nline + " F8  Toggle HardwareModeSwitch " +
        ...
        public void ListSupportedDisplayModes()
        {
            ...
            "  Current Mode: " + gd.Adapter.CurrentDisplayMode +
            ndnline
            ;

            Console.WriteLine(supportedModes);
            int counter = 0;
            counter = 0;
            ...
            for (int i = 0; i < dms.Count(); i++)
            {
                ...
                if (now.Format == starteddm.Format && now.AspectRatio == starteddm.AspectRatio)
                {
                    ...
                    if (isgood)
                    {
                        ...
                        bestMode = "  (" + i + ")-->(" + (bestDisplayModes.Count() - 1) + ")" + now + " aspct " + now.AspectRatio + " format " + now.Format + "\n";
                        Console.WriteLine(bestMode);
                    }
                }
            }
        ...
        public class RecordDiff
        {
            ...
            public static string DisplayCurrentStatus(GraphicsDeviceManager gdm, GraphicsDevice gd, GameWindow Window)
            {
                ...
                // common
                s += nl + (("  gdm.HardwareModeSwitch  ".PadRight(paddingamount,c)) + gdm.HardwareModeSwitch);
                s += nl + (("  gd.Adapter.IsWideScreen    ".PadRight(paddingamount, c)) + gd.Adapter.IsWideScreen);
                s += nl + (("  gdm.PreferredBackBufferFormat  ".PadRight(paddingamount, c)) + gdm.PreferredBackBufferFormat);
                s += nl + (("  gdm.PreferredBackBuffer WH: ".PadRight(paddingamount, c)) + " Width:" + gdm.PreferredBackBufferWidth + " Height:" + gdm.PreferredBackBufferHeight);
                s += nl + (("  Window.ClientBounds    ".PadRight(paddingamount, c)) + Window.ClientBounds);
                s += nl + (("  gd.PresentationParameters.DisplayOrientation ".PadRight(paddingamount, c)) + gd.PresentationParameters.DisplayOrientation);
                s += nl + (("  gd.PresentationParameters.Bounds   ".PadRight(paddingamount, c)) + gd.PresentationParameters.Bounds);
                s += nl + (("  gd.PresentationParameters.BackBuffer WH: ".PadRight(paddingamount, c)) + "  Width:" + gd.PresentationParameters.BackBufferWidth + " Height:" + gd.PresentationParameters.BackBufferHeight);
                s += nl + (("  gd.PresentationParameters.BackBufferFormat   ".PadRight(paddingamount, c)) + gd.PresentationParameters.BackBufferFormat);
                s += nl + (("  gd.Viewport   ".PadRight(paddingamount, c)) + gd.Viewport);
                s += nl + (("  gd.Viewport.TitleSafeArea    ".PadRight(paddingamount, c)) + gd.Viewport.TitleSafeArea);
                s += nl + (("  gd.DisplayMode.Width, Height    ".PadRight(paddingamount, c)) + "Width:" + gd.DisplayMode.Width + " Height:" + gd.DisplayMode.Height);
                s += nl + (("  gd.DisplayMode.TitleSafeArea   ".PadRight(paddingamount, c)) + gd.DisplayMode.TitleSafeArea);
                s += nl + (("  gd.Adapter.CurrentDisplayMode WH:  ".PadRight(paddingamount, c)) + "Width:" + gd.Adapter.CurrentDisplayMode.Width + " Height:" + gd.Adapter.CurrentDisplayMode.Height);
                s += nl + (("  gd.Adapter.CurrentDisplayMode.TitleSafeArea  ".PadRight(paddingamount, c)) + gd.Adapter.CurrentDisplayMode.TitleSafeArea);
